...
$stmt = $conn->query("SELECT * FROM deparment");
...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && !empty($_GET['id'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM deparment WHERE id = ?");
...
if (isset($_GET['action']) && $_GET['action'] === 'edit' && !empty($_GET['id'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM deparment WHERE id = ?");
    $stmt->execute([$id_to_edit]);
    $department_to_edit = $stmt->fetch();

    if ($department_to_edit) {
        $id_edit = $department_to_edit['id'];
        $name_edit = $department_to_edit['name'];
        $description_edit = $department_to_edit['description'];
        $is_editing = true;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($name)) {
        if (!empty($id)) {
            ...
            $stmt = $conn->prepare("UPDATE deparment SET name = ?, description = ? WHERE id = ?");
            $stmt->execute([$name, $description, $id]);
        } else {
            // Insert new department
            $stmt = $conn->prepare("INSERT INTO deparment (name, description) VALUES (?, ?)");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_department'])) {
    ...
    if (!empty($id) && !empty($name) && !empty($description)) {
        try {
            ...
            die("Error updating department: " . $e->getMessage());
        }
            ...
            <div class="container-fluid">
                <div class="d-flex justify-content-between align-items-center mb-4 pb-2 border-bottom" style="padding: 5px;">
                    ...
                    <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addDepartmentModal">
                        <i class="fas fa-plus mr-2"></i> Department
                    </button>
                </div>
                ...
                <div class="modal fade" id="addDepartmentModal" tabindex="-1">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <form method="post" action="departement.php" id="addDepartmentForm">
                                ...
                                <div class="d">
                                    ...
                                    <textarea id="description" name="description" class="form-control" required placeholder="Computer department description"></textarea>
                                </div>
                                <div class="d">
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <form method="post" id="editDepartmentForm">
                                ...
                                <div class="d">
                                    <label for="name">Department Name</label>
                                    <input type="text" id="edit_name" name="name" class="form-control" required placeholder="Computer">
                                </div>
                                <div class="d">
                                    <label for="description">Description</label>
                                    <textarea id="edit_description" name="description" class="form-control" required placeholder="Computer department description"><?= htmlspecialchars($departments['description']) ?></textarea>
                                </div>

                                <div class="d">
                                    <button type="submit" class="btn btn-primary" name="edit_department">Update Department</button>
                                    <button type="reset" class="btn btn-secondary">Reset</button>
                                </div>
                            </form>
                        </div>
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-6">
                            <div class="card border-0 shadow-sm p-3">
                                ...
                                <h3 class="fw-bold text-dark mb-0"><?php echo $totaldepartment  ?></h3>
                            </div>
                        </div>
                        <div class="col-6"></div>
                    </div>
                                ...
                                <?php foreach ($departments as $department): ?>
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <button data-bs-toggle="modal" data-bs-target="#editDepartmentModal" title="modify" class="btn btn-sm btn-outline-info edit-btn" data-id="<?= $department['id'] ?>" data-name="<?= $department['name'] ?>" data-description="<?= $department['description'] ?>"><i class="fas fa-edit"></i></button>
                                            <a href="departement.php?action=delete&id=<?= $department['id'] ?>" title="delete" onclick="return confirm('do you want to delete this department ? ')" class="btn btn-sm btn-outline-danger delete-btn"><i
